...
class Labelme2coco_keypoints():
    ...
    def _annotation(self, bboxes_list, keypoints_list, json_path):
        ...
        for object in bboxes_list:
            ...
            keypoints_list.sort(key=lambda x:keypoint_dict[x['label']])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input", "--i", help="json file path (labelme)", type=str, required=True,default=r'G:\X-AnyLabeling\test')
    parser.add_argument("--output", "--o", help="output file path (coco format)", type=str, required=True,default=r'G:\X-AnyLabeling\test\coco')
    parser.add_argument("--join_num", "--j", help="number of join", type=int, required=True,default=5)
